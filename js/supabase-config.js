...
async function loginSellerToSupabase(username, password) {
  ...
  try {
    ...
    const email = rawUsername === 'admin@mail.com' ? 'admin@mail.com' : `${safeUsername}@umkmpanjalu.web.id`;
    ...
    if (dbError || !sellerData) {
      ...
      const isSuperAdmin = authData.user.email === 'admin@mail.com';
    ...
    return {
      ...
      seller: sellerData,
      role: authData.user.email === 'admin@mail.com' ? 'super_admin' : 'seller'
    };
  } catch (err) {
    return { success: false, message: err.message };
  }
